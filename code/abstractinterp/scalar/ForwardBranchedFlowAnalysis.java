...
public abstract class ForwardBranchedFlowAnalysis<N extends Unit, A> extends ForwardBranchedFlowBasic<N, A> {

	public ForwardBranchedFlowAnalysis(DirectedGraph<N> graph) {
		super(graph);
		//populate order
		unitToAfterFallFlow = new HashMap<N, List<A>>(graph.size() * 2 + 1, 0.7f);
		unitToAfterBranchFlow = new HashMap<N, List<A>>(graph.size() * 2 + 1, 0.7f);
