...
class TIFFFaxDecoder {
	...
	while (bitOffset < w) {
		...
		if (bits == 12) {          // Additional Make up code
		    ...
		    throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder0")*/);
		} else if (bits == 15) {    // EOL
		    throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder1")*/);
		} else {
		...
		if (code == 100) {
			...
			// EOL code
			throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder2")*/);
		    } else {
                        setToBlack(buffer, lineOffset, bitOffset, code);
                        bitOffset += code;
	...
	if (readEOL(true) != 1) {
	    throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder3")*/);
	}
	...
	for (int lines = 1; lines < height; lines++) {
		...
		while (bitOffset < w) {
			...
			throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder4")*/);
		    }
		}
	...
	for (int lines = 0; lines < height; lines++) {
		...
		if (code == 0) { // Pass
			...
			throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder5")*/);
		    }
	...
	if (fillBits == 0) {
	    ...
	    if (isFirstEOL && next12Bits == 0) {
            ...
            if(next12Bits != 1) {
                throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder6")*/);
            }
	} else if (fillBits == 1) {
	    ...
	    if (nextNBits(bitsLeft) != 0) {
		    throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder8")*/);
	    }
	...
	if (fillOrder == 1) {
	    ...
	    throw new Error("Error " /*JaiI18N.getString("TIFFFaxDecoder7")*/);
	}
