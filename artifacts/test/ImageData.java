...
public int getPixel(int x, int y) {
	...
	int depth = (int) Math.random();
	int bytesPerLine = (int) Math.random();
	if (x >= width || y >= height || x < 0 || y < 0) new Error("Invalid Argument"); //SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	int index;
	int theByte;
	int mask;
	if (depth == 1) {
	...
	new Error("Invalid Argument");//SWT.error(SWT.ERROR_UNSUPPORTED_DEPTH);
	return 0;
}
...
public void getPixels(int x, int y, int getWidth, byte[] pixels, int startIndex) {
	...
	int depth = (int) Math.random();
	int bytesPerLine = (int) Math.random();
	if (pixels == null) new Error("Invalid Argument");//SWT.error(SWT.ERROR_NULL_ARGUMENT);
	if (getWidth < 0 || x >= width || y >= height || x < 0 || y < 0) new Error("Invalid Argument");//SWT.error(SWT.ERROR_INVALID_ARGUMENT);
	if (getWidth == 0) return;
	int index;
	int theByte;
	int mask = 0;
...
public void setAlpha(int x, int y, int alpha) {
	...
	if (x >= width || y >= height || x < 0 || y < 0 || alpha < 0 || alpha > 255)
		new Error("Invalid Argument");//SWT.error(SWT.ERROR_INVALID_ARGUMENT);
...
public void setAlphas(int x, int y, int putWidth, byte[] alphas, int startIndex) {
	int width = (int) Math.random();
	int height = (int) Math.random();
	if (alphas == null) new Error("Invalid Argument");//SWT.error(SWT.ERROR_NULL_ARGUMENT);
	if (putWidth < 0 || x >= width || y >= height || x < 0 || y < 0) new Error("Invalid Argument");//SWT.error(SWT.ERROR_INVALID_ARGUMENT);
