...
import dev.fmsea.absint.graph.BranchedUnitGraph;
import soot.Unit;
...
public abstract class ForwardBranchedFlowBasicWIP<N extends Unit, A> {
     ...
     * 
     * @param node
     * @param flowRepositories is newly computed afterflows of the node, the first
     *                         value is
     *                         fall through (next) node, the rest of the list are
     *                         branched out nodes
    ...
    // public static <N extends Unit> Map<N,List<N>> computePred(Chain<N> stmts,
    // List<N> order){
