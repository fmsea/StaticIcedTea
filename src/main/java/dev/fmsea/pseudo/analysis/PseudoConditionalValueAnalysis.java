...
import dev.fmsea.disjoint.analysis.ValueAnalysis;
...
import dev.fmsea.driver.StartAnalysisScript;
...
public class PseudoConditionalValueAnalysis extends ValueAnalysis {
    ...
    // @Override
    public void report() {
        System.out.println("Done in " + time + " fn " + Timers.v().totalFlowNodes + ", fc"
        ...
        if (StartPseudoConditionalValue.writeTime) {
            try {
                ...
                String timeDataFile = StartPseudoConditionalValue.path + "/time/"
                        + StartPseudoConditionalValue.className + "_" + StartPseudoConditionalValue.methodId + "_"
                        + StartPseudoConditionalValue.domain + ".txt";
    ...
    // copy(in1,out); //if in2 is infeasible then copy in1 to out, does not matter
    // if in1 is feasible or not
