...
public class OctagonGeVariableRule extends RewriteRule {
    public boolean canRewrite(TADR expr) {
        return ((expr instanceof GeCmp) &&
                (((GeCmp)expr).left instanceof Variable) &&
                (((GeCmp)expr).right instanceof Variable));
    }
    ...
    public Stream<TADR> rewrite(GeCmp expr, Function<Local, Interval32Box> lookup) {
        return Stream.of(TADR.newLeExpr(expr.right, TADR.newAddExpr(expr.left, TADR.newValue(0))));
    }
