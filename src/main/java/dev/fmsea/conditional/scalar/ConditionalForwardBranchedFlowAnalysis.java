...
package dev.fmsea.conditional.scalar;
...
public abstract class ConditionalForwardBranchedFlowAnalysis<A> extends BranchedFlowAnalysis<Unit, A> {
    ...
    public ConditionalForwardBranchedFlowAnalysis(UnitGraph graph, List<ConditionalInfo> branchList) {
        ...
        condToLine = new HashMap<Unit, Integer>(); // a map of conditional units to the number of that conditional
                                                   // statement. //the first conditional statement is the 0 conditional
                                                   // statement
    ...
    // Accumulate the previous afterFlow sets. //This method is called twice in
    // fixed point algorithm
    // The first time is before the flowThrough is called, to accumulate the
    // previousAfterFlows for that node
    // the second time is after the flowThough is caled to accumulate teh afterFlows
    // for that node
    // if the iterative algorithm only goes through the graph one time without
    // loops, then the previousAfterFlows will always be empty
    ...
    // private void specialDFS(Map<Unit, Integer> condToLine, List<Unit>
    // orderedWorkList, Unit root){ //special depth first search to get the
    // orderedWorkingList by only traversing down the branches we select to go down
    //
    // HashSet<Unit> visited = new HashSet<Unit>();
    // visited.add(root);
    //
    // //create an empty stack of Unit
    // int last = 0; //corresponds to the level of the stack we are at. always 1
    // more than the level we are at
    // ArrayList <Unit> stmtStack = new ArrayList<Unit>(); //add and remove from end
    // to implement as stack
    // //add the root to the stack
    // ArrayList <Integer> indexStack = new ArrayList<Integer>(); //each element
    // corresponds to the next child for the stmtStack
    //
    ...
    protected void doAnalysis() {
        final Map<Unit, Integer> numbers = new HashMap<Unit, Integer>(); // a map of each unit or line in a program to
                                                                         // its line number
        ...
        // List<Unit> orderedUnits = (new
        // PseudoTopologicalOrderer<Unit>()).newList(graph, false); //ordering the nodes
        // in the graph topologically
        List<Unit> orderedWorkList = myOrderer.newList(graph, false);
        // List<Unit> orderedWorkList = new ArrayList<Unit>(); //will contain a
        // pseudotopologically ordered set of the units we wish to process
        ...
        Map<Unit, ArrayList<A>> unitToIncomingFlowSets = new HashMap<Unit, ArrayList<A>>(graph.size() * 2 + 1, 0.7f); // mapping
                                                                                                                      // each
                                                                                                                      // unit
                                                                                                                      // to
                                                                                                                      // incoming
        ...
        {
            Chain<Unit> sl = ((UnitGraph) graph).getBody().getUnits(); // getting all the units in the graph and putting
                                                                       // them in sl
            for (Unit s : graph) {
                // changedUnits.add(s); //we want the changedUnits to initially be all the units

                unitToBeforeFlow.put(s, newInitialFlow()); // the flowBefore each unit is set to an initial flow. This
                                                           // is a hash map of unit to whatever our flow type is and is
                                                           // slightly different from our unitToIncomingFlow, which maps
                                                           // to a list of incoming flows
                // the unitToBeforeFlow always maps to one incoming flow. the unitToIncomingFlow
                // can map to a list of incoming flow, which sometimes is merged into one flow
                // which is put in unitToBeforeFlow
                ...
                if (s.fallsThrough()) {
                    ...
                    unitToAfterFallFlow.put(s, new ArrayList<A>()); // if s doesn't fall through no need to put in
                                                                    // incoming flow set of successor and just put an
                                                                    // empty list
                ...
                unitToAfterBranchFlow.put(s, l); // now the afterBranchFlow of s is initiallized as a new initial flow,
                                                 // and put in the map
        ...
        // System.out.println("I'm printing out the unit to Incoming Flow because my
        // program is annoying");
        // for (Unit u: unitToIncomingFlowSets.keySet()){
        // System.out.println("Unit: " + u + " " + "Incoming Flow Set: " +
        // System.identityHashCode(unitToIncomingFlowSets.get(u)));
        // }
        ...
        {
            List<Object> previousAfterFlows = new ArrayList<Object>(); // used to keep track of the previous After Flows
                                                                       // for each node. for the first iteration, this
                                                                       // is going to be the empty set
            List<Object> afterFlows = new ArrayList<Object>(); // used to keep track of the after Flows for each list of
                                                               // objects. will be compared to the previousAfter flows
                                                               // to see if the units after flow was changed
            ...
            while (!changedUnits.isEmpty()) {
                // System.out.println("At the beginning of the loop for the iterative
                // algorithm!!");
                ...
                accumulateAfterFlowSets(s, previousFlowRepositories, previousAfterFlows); // just updating the
                                                                                          // previousAfterFlow to be the
                                                                                          // current afterFlow, before
                                                                                          // we've called the
                                                                                          // flowThrough method
                ...
                // Compute and store beforeFlow
                {
                    ...
                    beforeFlow = getFlowBefore(s); // getting the current before flow of the unit we are at (in first
                                                   // iteration this should just be whatever initialFlow() returns)

                    // the unitToIncomingFlow sets was initialized to be connected to the incoming
                    // flow from previous branches and fallOuts
                    // if in the last iteration we changed the afteflow of these previous precsessor
                    // nodes we need to copy that new inflow into our unitToFlowBefore

                    if (preds.size() == 1) // if there is only one incoming flow, just copy it to the current beforeFlow
                        copy(preds.get(0), beforeFlow);
                    else if (preds.size() != 0) { // otherwise, copy the first incmoing flow, then copy all the branches
                                                  // that are merging into this unit
                    ...
                    if (isHead && preds.size() != 0) // if it's the head and has no pred nodes, then mergeInto the
                                                     // beforeFlow (not really sure what this is doing), i feel like if
                                                     // its the head it should just have no beforeFlow
                ...
                // Compute afterFlow and store it.
                {
                    ...
                    // System.out.println("in flow " + beforeFlow + "before fall " + afterFallFlow +
                    // " before branch " + afterBranchFlow );
                ...
                // System.out.println("printing out after fall again " +
                // unitToAfterFallFlow.get(s).get(0));
                // }
                ...
                if (!afterFlows.equals(previousAfterFlows)) { // whatever you are flowing through needs to override
                                                              // Objects equals method otherwise this won't work
                    ...
                    // System.out.println("The previous after flow is: " +
                    // previousAfterFlows.toString());
        ...
        // System.out.println("Fall Flow List Location: " +
        // System.identityHashCode(unitToAfterFallFlow.get(u)));
        // for (int i = 0; i < unitToAfterFallFlow.get(u).size(); i++){
        // System.out.println("element: " + i + " " +
        // System.identityHashCode(unitToAfterFallFlow.get(u).get(0)));
        // }
        // System.out.println("Branch Flow List Location: " +
        // System.identityHashCode(unitToAfterBranchFlow.get(u)));
        // for (int i = 0; i < unitToAfterBranchFlow.get(u).size(); i++){
        // System.out.println("element: " + i + " " +
        // System.identityHashCode(unitToAfterBranchFlow.get(u).get(0)));
        // }
        // }
