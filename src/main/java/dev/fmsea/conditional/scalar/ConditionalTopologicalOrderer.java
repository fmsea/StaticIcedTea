...
public class ConditionalTopologicalOrderer<N> implements Orderer<N> {
    ...
    protected final List<N> computeOrder(DirectedGraph<N> g, boolean reverse) {
        ...
        final int n = g.size();
        visited = Collections.newSetFromMap(new IdentityHashMap<N, Boolean>(n * 2 + 1));// new HashMap((3 * g.size()) /
                                                                                        // 2, 0.7f);
    ...
    private boolean specialCond(boolean branch, N node) { // returns true if the node we are at is a conditional node,
                                                          // and it only takes the branch we pass in
    ...
    protected final void visitNode(N startStmt) {
        ...
        // check if startStmt is an if statement and we only want the falloutbranch,
        // which is the false branch and weirdly the first child
        ...
        if (onlyBranch) {
            ...
            indexStack[last++] = -1; // corresponds to the node at that same index in the stmtStack. always 1 less
                                     // then the next child we want to visit for that node.
        }
