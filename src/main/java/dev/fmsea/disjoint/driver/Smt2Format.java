...
public class Smt2Format {
    ...
    public static void main(String[] args) throws IOException {
        // String file1Name = "dfaWSolver/disjoint/domain/test1.txt";
        ...
        // do the pairwise comparisons of the same classNames but with
        // different domains
        for (String className : classNames) {
            for (String symb : symbolicOn) {
                for (int i = 0; i < domainNames.length; i++) {
                    for (int j = i + 1; j < domainNames.length; j++) {
                        try {
                            String file1Name = className + "_" + symb + "_" + domainNames[i];
                            String file2Name = className + "_" + symb + "_" + domainNames[j];
                            ;
        ...
        // pairwise comparison of the same class name
        // and the same domain only for different symbolic keys

        for (String className : classNames) {
            for (String domain : domainNames) {
                for (int i = 0; i < symbolicOn.length; i++) {
                    for (int j = i + 1; j < symbolicOn.length; j++) {
                        try {
                            String file1Name = className + "_" + symbolicOn[i] + "_" + domain;
                            String file2Name = className + "_" + symbolicOn[j] + "_" + domain;
                            System.out.println(file1Name + " " + file2Name);
                            new Smt2Format(file1Name, file2Name);
                        } catch (IOException e) {
                            // TODO Auto-generated catch block
                            e.printStackTrace();
        ...
        String file1Name = classNames[0] + "_sY_" + domainNames[0];
        String file2Name = classNames[0] + "_sN_" + domainNames[1];
        new Smt2Format(file1Name, file2Name);
    }
    ...
    private String writeConstraint(Map<String, String> stmtTo, String constraint, String var, String formula)
            throws IOException {
