package dev.fmsea.disjoint.domain.reader;
...
import dev.fmsea.disjoint.domain.IntervalPredicate;
...
public class DomainInstantiator extends DisjointDomainBaseVisitor {
    ...
    @Override
    public BaseElement visitIntervals(DisjointDomainParser.IntervalsContext ctx) {
        ...
        // domain.done(); unimplemented method that should check whether the domain is
        // complete
        // we do it here ...
                ...
                for (BaseElement be2 : domain.getBaseElements()) {
                    if (!be1.equals(be2)) {
                        ...
                        if (sat) {
                            ...
                            return; // we're done -- found at lest one pair of predicates that are not pair-wise
                                    // disjoint
                        }
                    }
                }
    ...
    @Override
    public Object visitInterval(DisjointDomainParser.IntervalContext ctx) {
    ...
    @Override
    public Object visitSingleton(DisjointDomainParser.SingletonContext ctx) {
        Set<BaseElement> ret = new HashSet<BaseElement>();
