...
public class MaxDifferenceBoundedState implements State {

    private PADO01DifferenceBoundedMatrix matrix;
    private static Logger LOGGER = LoggerFactory.getLogger(MaxDifferenceBoundedState.class);
     ...
     */
    public MaxDifferenceBoundedState(Set<Local> locals, boolean top) {
     ...
     */
    public MaxDifferenceBoundedState(MaxDifferenceBoundedState state) {
     ...
     */
    public MaxDifferenceBoundedState(PADO01DifferenceBoundedMatrix matrix) {
        this.matrix = matrix;
    }
     ...
     */
    public State copy() {
        return new MaxDifferenceBoundedState(this);
    }
    ...
    public void copyTo(State destination) {
        if (destination instanceof MaxDifferenceBoundedState) {
            this.copyTo((MaxDifferenceBoundedState)destination);
        } else {
     ...
     * @param destination
     */
    public void copyTo(MaxDifferenceBoundedState destination) {
    ...
    public boolean add(Local source,
                       Local target,
                       PADO01Constraint constraint,
                       MaxDifferenceBoundedState inState) {
        boolean added = false;
    ...
    public void mergeWith(State inState) {
        if (inState instanceof MaxDifferenceBoundedState) {
            this.mergeWith((MaxDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("Invalid type for mergeWith");
        }
    }

    public void mergeWith(MaxDifferenceBoundedState inState) {
        this.matrix.union(inState.matrix);
    }

    public void widenWith(State inState) {
        if (inState instanceof MaxDifferenceBoundedState) {
            this.widenWith((MaxDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("Invalid type for widenWith");
        }
    }

    public void widenWith(MaxDifferenceBoundedState inState) {
        this.matrix.widenWith(inState.matrix);
    }

    public boolean isSubset(State inState) {
        if (inState instanceof MaxDifferenceBoundedState) {
            return this.isSubset((MaxDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("invalid type for isSubset");
        }
    }

    public boolean isSubset(MaxDifferenceBoundedState inState) {
    ...
    public void updateState(Local lVar, State inState, Value v) {
        if (inState instanceof MaxDifferenceBoundedState) {
            this.updateState(lVar, (MaxDifferenceBoundedState) inState, v);
        } else {
            throw new RuntimeException("invalid type for updateState");
        }
    }

    public void updateState(Local lVar, MaxDifferenceBoundedState inState, Value v) {
    ...
    public void updateState(Local lVar, MaxDifferenceBoundedState inState, IntConstant c) {
        this.forget(lVar);
        this.add(lVar, ZERO, PADO01Constraint.of(c.value));
        this.add(ZERO, lVar, PADO01Constraint.of(c.value * -1));
    }

    public void updateState(Local lVar, MaxDifferenceBoundedState inState, Local l) {
        this.forget(lVar);
    ...
    public boolean updateCond(State inState, Value left, Value right, PredicateType type) {
        if (inState instanceof MaxDifferenceBoundedState) {
            return this.updateCond((MaxDifferenceBoundedState) inState,
                                   left,
                                   right,
                                   type);
        } else {
    ...
    public boolean updateCond(MaxDifferenceBoundedState inState,
                              Value left,
                              Value right,
                              PredicateType type) {
    ...
    public boolean equals(Object o) {
        boolean equal = false;
        if (o != null && o instanceof MaxDifferenceBoundedState) {
            equal = this.equals((MaxDifferenceBoundedState) o);
        }
        return equal;
    }

    public boolean equals(MaxDifferenceBoundedState other) {
