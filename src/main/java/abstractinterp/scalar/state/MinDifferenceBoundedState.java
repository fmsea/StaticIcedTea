...
public class MinDifferenceBoundedState implements State {

    private PADO01DifferenceBoundedMatrix matrix;
    private static Logger LOGGER = LoggerFactory.getLogger(MinDifferenceBoundedState.class);
     ...
     */
    public MinDifferenceBoundedState(Set<Local> locals, boolean top) {
     ...
     */
    public MinDifferenceBoundedState(MinDifferenceBoundedState state) {
     ...
     */
    public MinDifferenceBoundedState(PADO01DifferenceBoundedMatrix matrix) {
        this.matrix = matrix;
    }
     ...
     */
    public State copy() {
        return new MinDifferenceBoundedState(this);
    }
    ...
    public void copyTo(State destination) {
        if (destination instanceof MinDifferenceBoundedState) {
            this.copyTo((MinDifferenceBoundedState)destination);
        } else {
     ...
     * @param destination
     */
    public void copyTo(MinDifferenceBoundedState destination) {
    ...
    public boolean add(Local source,
                       Local target,
                       PADO01Constraint constraint,
                       MinDifferenceBoundedState inState) {
        boolean added = false;
    ...
    public void mergeWith(State inState) {
        if (inState instanceof MinDifferenceBoundedState) {
            this.mergeWith((MinDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("Invalid type for mergeWith");
        }
    }

    public void mergeWith(MinDifferenceBoundedState inState) {
    ...
    public void widenWith(State inState) {
        if (inState instanceof MinDifferenceBoundedState) {
            this.widenWith((MinDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("Invalid type for widenWith");
        }
    }

    public void widenWith(MinDifferenceBoundedState inState) {
        this.matrix.computeClosure();
        inState.matrix.computeClosure();
    ...
    public boolean isSubset(State inState) {
        if (inState instanceof MinDifferenceBoundedState) {
            return this.isSubset((MinDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("invalid type for isSubset");
        }
    }

    public boolean isSubset(MinDifferenceBoundedState inState) {
        this.matrix.computeClosure();
        inState.matrix.computeClosure();
    ...
    public void updateState(Local lVar, State inState, Value v) {
        if (inState instanceof MinDifferenceBoundedState) {
            this.updateState(lVar, (MinDifferenceBoundedState) inState, v);
        } else {
            throw new RuntimeException("invalid type for updateState");
        }
    }

    public void updateState(Local lVar, MinDifferenceBoundedState inState, Value v) {
    ...
    public void updateState(Local lVar, MinDifferenceBoundedState inState, IntConstant c) {
        this.forget(lVar);
        this.add(lVar, ZERO, PADO01Constraint.of(c.value));
        this.add(ZERO, lVar, PADO01Constraint.of(c.value * -1));
    }

    public void updateState(Local lVar, MinDifferenceBoundedState inState, Local l) {
        this.forget(lVar);
    ...
    public boolean updateCond(State inState, Value left, Value right, PredicateType type) {
        if (inState instanceof MinDifferenceBoundedState) {
            return this.updateCond((MinDifferenceBoundedState) inState,
                                   left,
                                   right,
                                   type);
        } else {
    ...
    public boolean updateCond(MinDifferenceBoundedState inState,
                              Value left,
                              Value right,
                              PredicateType type) {
    ...
    public boolean equals(Object o) {
        boolean equal = false;
        if (o != null && o instanceof MinDifferenceBoundedState) {
            equal = this.equals((MinDifferenceBoundedState) o);
        }
        return equal;
    }

    public boolean equals(MinDifferenceBoundedState other) {
        this.matrix.computeClosure();
