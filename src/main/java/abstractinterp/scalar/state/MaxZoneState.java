...
public class MaxZoneState implements State {
     ...
     */
    public MaxZoneState(ZoneDifferenceBoundedMatrix matrix) {
        this.matrix = matrix;
    }
    ...
    public void widenWith(State inState, Set<Integer> steps) {
        if (inState instanceof MaxZoneState) {
            this.widenWith((MaxZoneState) inState, steps);
        } else {
            throw new RuntimeException("Invalid type for widenWith");
        }
    }

    public void widenWith(MaxZoneState inState, Set<Integer> steps) {
    ...
    public void updateState(Local lVar, MaxZoneState inState, LongConstant c) {
        this.forget(lVar);
        this.add(lVar, ZERO, Constraint.of(c.value));
        this.add(ZERO, lVar, Constraint.of(c.value * -1));
    }
