...
public class IncDifferenceBoundedState implements State {

    protected PADO01DifferenceBoundedMatrix matrix;
    private static Logger LOGGER = LoggerFactory.getLogger(IncDifferenceBoundedState.class);
     ...
     */
    public IncDifferenceBoundedState(Set<Local> locals, boolean top) {
     ...
     */
    public IncDifferenceBoundedState(IncDifferenceBoundedState state) {
     ...
     */
    public IncDifferenceBoundedState(PADO01DifferenceBoundedMatrix matrix) {
        this.matrix = matrix;
    }
     ...
     */
    public State copy() {
        return new IncDifferenceBoundedState(this);
    }
    ...
    public void copyTo(State destination) {
        if (destination instanceof IncDifferenceBoundedState) {
            this.copyTo((IncDifferenceBoundedState)destination);
        } else {
     ...
     * @param destination
     */
    public void copyTo(IncDifferenceBoundedState destination) {
    ...
    public void add(Local source,
                    Local target,
                    PADO01Constraint constraint,
                    IncDifferenceBoundedState inState) {
    ...
    public void assign(Local source,
        ...
        this.add(source,
                 target,
                 PADO01Constraint.of(box.upperBound()),
                 inState);
        this.add(target,
                 source,
                 PADO01Constraint.of(box.lowerBound().map(b -> b * -1)),
                 inState);
    }
    ...
    public void mergeWith(State inState) {
        if (inState instanceof IncDifferenceBoundedState) {
            this.mergeWith((IncDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("Invalid type for mergeWith");
        }
    }

    public void mergeWith(IncDifferenceBoundedState inState) {
        this.matrix.union(inState.matrix);
    }

    public void widenWith(State inState) {
        if (inState instanceof IncDifferenceBoundedState) {
            this.widenWith((IncDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("Invalid type for widenWith");
        }
    }

    public void widenWith(IncDifferenceBoundedState inState) {
        this.matrix.widenWith(inState.matrix);
    }

    public boolean isSubset(State inState) {
        if (inState instanceof IncDifferenceBoundedState) {
            return this.isSubset((IncDifferenceBoundedState) inState);
        } else {
            throw new RuntimeException("invalid type for isSubset");
        }
    }

    public boolean isSubset(IncDifferenceBoundedState inState) {
    ...
    public void updateState(Local lVar, State inState, Value v) {
        if (inState instanceof IncDifferenceBoundedState) {
            this.updateState(lVar, (IncDifferenceBoundedState) inState, v);
        } else {
            throw new RuntimeException("invalid type for updateState");
        }
    }

    public void updateState(Local lVar, IncDifferenceBoundedState inState, Value v) {
        this.forget(lVar);
    ...
    public boolean updateCond(State inState, Value left, Value right, PredicateType type) {
        if (inState instanceof IncDifferenceBoundedState) {
            return this.updateCond((IncDifferenceBoundedState) inState,
                                   left,
                                   right,
                                   type);
        } else {
    ...
    public boolean updateCond(IncDifferenceBoundedState inState,
                              Value left,
                              Value right,
                              PredicateType type) {
    ...
    public boolean equals(Object o) {
        boolean equal = false;
        if (o != null && o instanceof IncDifferenceBoundedState) {
            equal = this.equals((IncDifferenceBoundedState) o);
        }
        return equal;
    }

    public boolean equals(IncDifferenceBoundedState other) {
