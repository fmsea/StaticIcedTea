...
public class ZoneState implements State {
     ...
     */
    public ZoneState(Set<Local> locals, boolean top) {
     ...
     */
    public ZoneState(ZoneState state) {
    ...
    public void copyTo(State destination) {
        if (destination instanceof ZoneState) {
            this.copyTo((ZoneState)destination);
        } else {
     ...
     * @param destination
     */
    public void copyTo(ZoneState destination) {
    ...
    public void mergeWith(State inState) {
        if (inState instanceof ZoneState) {
            this.mergeWith((ZoneState) inState);
        } else {
            throw new RuntimeException("Invalid type for mergeWith");
        }
    }

    public void mergeWith(ZoneState inState) {
    ...
    public void widenWith(State inState) {
        if (inState instanceof ZoneState) {
            this.widenWith((ZoneState) inState);
        } else {
            throw new RuntimeException("Invalid type for widenWith");
        }
    }

    public void widenWith(ZoneState inState) {
        this.matrix.computeClosure();
        inState.matrix.computeClosure();
    ...
    public boolean isSubset(State inState) {
        if (inState instanceof ZoneState) {
            return this.isSubset((ZoneState) inState);
        } else {
            throw new RuntimeException("invalid type for isSubset");
        }
    }

    public boolean isSubset(ZoneState inState) {
        this.matrix.computeClosure();
        inState.matrix.computeClosure();
    ...
    public void updateState(Local lVar, State inState, Value v) {
        if (inState instanceof ZoneState) {
            this.updateState(lVar, (ZoneState) inState, v);
        } else {
            throw new RuntimeException("invalid type for updateState");
        }
    }

    public void updateState(Local lVar, ZoneState inState, Value v) {
    ...
    public void updateState(Local lVar, ZoneState inState, IntConstant c) {
        this.forget(lVar);
        this.add(lVar, ZERO, Constraint.of(c.value));
        this.add(ZERO, lVar, Constraint.of(c.value * -1));
    }

    public void updateState(Local lVar, ZoneState inState, LongConstant c) {
        this.forget(lVar);
        this.add(lVar, ZERO, Constraint.of(c.value));
        this.add(ZERO, lVar, Constraint.of(c.value * -1));
    }

    public void updateState(Local lVar, ZoneState inState, Local l) {
        this.forget(lVar);
    ...
    public boolean updateCond(State inState, Value left, Value right, PredicateType type) {
        if (inState instanceof ZoneState) {
            return this.updateCond((ZoneState) inState,
                                   left,
                                   right,
                                   type);
        } else {
    ...
    public boolean updateCond(ZoneState inState,
                              Value left,
                              Value right,
                              PredicateType type) {
    ...
    public boolean equals(Object o) {
        boolean equal = false;
        if (o != null && o instanceof ZoneState) {
            equal = this.equals((ZoneState) o);
        }
        return equal;
    }

    public boolean equals(ZoneState other) {
        this.matrix.computeClosure();
